# 思路
'''
    在脚本所在目录下创建 美女图集 文件夹并进入文件夹，如果存在则忽略创建直接进入目录,创建重试函数，为了规避网络问题导致的 tls 报错，并且使用脚本实现需求，将整个代码发给我，并且将代码分段，每段子树限制 1800 字，第一次宪法第一段代码，之后则在我每次回复 继续 则发送下一段代码
...
for i in range(1, a_pager + 1):
    ...
    for j in range(b_count):
        b_name = os.path.join(a_name, b_names[j].replace(' ','-').replace('-–-','-').replace('#',''))
        # 在当前目录下创建对应的 b_name 目录并进入目录，如果存在则不创建目录直接进入目录
        ...
        for k in range(1, b_pager + 1):
            ...
            for l in range(c_count):
                c_name = os.path.join(b_name, c_names[l].replace(' ','-').replace('-–-','-').replace('#',''))
                # 创建对应的 c_name 目录并进入目录，如果存在则不创建目录直接进入目录
