...
'''
    ...
    拼接 a_url_cut 和 a_pager 得到类似 https://www.248.one/c49p2.aspx 的拼接其中 p2 代表第一页之后的拼接以此类推，得到拼接 a_url_pager
...
for i in range(1, a_pager + 1):
    ...
    for j in range(0,b_count):
        # 在当前目录下创建 b_title 目录，存在则忽略，并进入目录
        b_name = os.path.join(a_name, b_title[j].replace(' ','-').replace('---','-').replace('@',''))
